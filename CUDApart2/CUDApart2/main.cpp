    ...
    if (test.compare("scan") == 0) { // test exclusive scan
        ...
        for (int i=0; i<3; i++) {
            if (useThrust)
                ...
                cudaTime = min(cudaTime, cudaScanThrust(inarray, inarray + N, resultarray));
            else
                //cudaTime = std::min(cudaTime, cudaScan(inarray, inarray+N, resultarray));
                cudaTime = min(cudaTime, cudaScan(inarray, inarray + N, resultarray));
        }
        ...
        for (int i=0; i<3; i++) {
            ...
            cudaTime = min(cudaTime, cudaFindRepeats(inarray, N, resultarray, &cu_size));
        }
